...
//        }
    ...
    public Result<?> pass(@RequestBody User user) {
        /*User user = userMapper.selectById((Integer) map.get("userId"));*/
        User userPwd = userMapper.selectByName(user.getUsername());
        QueryWrapper<User> queryWrapper = new QueryWrapper<>();
        queryWrapper.eq("username", user.getUsername());
        queryWrapper.eq("password", userPwd.getPassword());
        User res = userMapper.selectAllByName(user.getUsername());

        // 判断密码是否正确
        if (!bCryptPasswordEncoder.matches(user.getAvatar(), userPwd.getPassword())) {
            return Result.error("-1", "密码错误");
        } else if (res == null) {
            return Result.error("-1", "用户名不正确");
        }
